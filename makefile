...
# The magic number in the end is the index where game data starts in sector 0 (see rom_emulator.asm or rom_production.asm)
SECTOR0_START_EMULATOR := 11000
SECTOR0_START := 10100
...
LAUNCHER_ADDRESS := 0x8001              # 0x8000 has the ROM_ID in emulator, so begin at 0x8001			
...
prepare_emulator: directories validate_config
	@echo "Compiling loader..."
	z88dk-z80asm -o=build/loader.bin -DEMULATOR=1 -m -l -b asm/loader.asm -DDZX0_ADDRESS=${DZX0_ADDRESS}
	mv asm/loader.lis build/loader.lis
	rm asm/loader.o
	scripts/synthetic_opcodes.sh build/loader.lis
	@echo "Loader compiled (`stat -f%z build/loader.bin` bytes)"
	...
	@echo "Creating game data..."
	node js/gamedata.js ${ROM_VERSION} `stat -f%z build/loader.bin` ${SECTOR0_START_EMULATOR} CHECK_CRC=${CHECK_CRC}
	@echo "Game data created"

	@echo "Compiling launcher with the final game data..."
	z88dk-z80asm -o=build/launcher.bin -DLOADER_SIZE=`stat -f%z build/loader.bin` -DLAUNCHER_ADDRESS=${LAUNCHER_ADDRESS} -DDZX0_ADDRESS=${DZX0_ADDRESS} -DEXEROM_DISABLE=${EXEROM_DISABLE} -DEMULATOR=1 -DCHECK_CRC=${CHECK_CRC} -m -l -b asm/launcher.asm
	mv asm/launcher.lis build/launcher.lis
	rm asm/launcher.o
	scripts/synthetic_opcodes.sh build/launcher.lis
	zx0 -f build/launcher.bin
...
emulator: directories prepare_emulator
	@echo "Compiling emulator version..."

	z88dk-z80asm -o=release/cartsim1.rom -DEMULATOR=1 -DSECTOR0_START=${SECTOR0_START_EMULATOR} -DLAUNCHER_ADDRESS=${LAUNCHER_ADDRESS} -DDZX0_ADDRESS=${DZX0_ADDRESS} -DEXEROM_DISABLE=${EXEROM_DISABLE} -m -l -b asm/rom_emulator_first.asm
	@mv asm/rom_emulator_first.lis build/rom_emulator_first.lis
	@rm asm/rom_emulator_first.o
	@scripts/synthetic_opcodes.sh build/rom_emulator_first.lis

	z88dk-z80asm -o=release/cartsim2.rom -DEMULATOR=1 -DLAUNCHER_ADDRESS=${LAUNCHER_ADDRESS} -DDZX0_ADDRESS=${DZX0_ADDRESS} -DEXEROM_DISABLE=${EXEROM_DISABLE} -m -l -b asm/rom_emulator_second.asm
	@mv asm/rom_emulator_second.lis build/rom_emulator_second.lis
	@rm asm/rom_emulator_second.o
	...
	@echo "Simulator version (release/cartsim1.rom and release/cartsim2.rom) compiled, use 'make run' to launch the emulator"
...
test:
	z88dk-z80asm -o=release/test.rom -DEMULATOR=1 -m -l -b asm/test.asm
